...
export interface IBaseable {
  ...
  Extract(token_group: TokenGroup, ctx: TokeniserContext): TokenGroupResponse<Statement>;
}
...
export abstract class Statement extends Component {
  ...
  static Parse(token_group: TokenGroup, ctx: TokeniserContext): TokenGroupResponse<Statement> {
    for (const possible of Statement.#possible)
      if (possible.Is(token_group)) return possible.Extract(token_group, ctx);
